...
# for password reset link and and for sending conventional data
...
class RegisterView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        email_body = 'Hi ' + user.username + ' Use the link below to verify your account: \n' + absurl
        data = {
...
class VerifyEmail(views.APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            return Response({'error': 'Invalid token request for new one.'}, status=status.HTTP_400_BAD_REQUEST)
...
class RequestPasswordResetAPIView(generics.GenericAPIView):
    serializer_class = RequestPasswordResetAPIViewSerializer
    ...
    def post(self, request):
        ...
        if User.objects.filter(email=email).exists():
            ...
            relativeLink = reverse('pasword-rest-confirm', kwargs={'uidb64': uidb64, 'token': token})
            ...
            Util.send_email(data)
        return Response({'success': 'A password reset link has been sent to your email. Click the link to reset '
    ...
    def get(self, request, uidb64, token):
        try:
            ...
            if not PasswordResetTokenGenerator().check_token(user, token):
                return Response({'error': 'Token not valid, Request a new one'}, status=status.HTTP_401_UNAUTHORIZED)
